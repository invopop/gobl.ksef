...
## Example XML request to /auth/xades-signature

An example request generated by the C# client is stored in `reference.xml`.

Another example request (different than above), with added whitespaces and newlines to make it human-readable, is stored in `reference-formatted.xml`. Note that the hash value inside the XML will be incorrect due to the added whitespace.
